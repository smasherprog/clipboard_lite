...
namespace Clipboard_Lite
{
    ...
    Clipboard_Manager::Clipboard_Manager()
    {
        ...
        Xatextplainutf2 =
            XInternAtom(d, "text/plain;charset=utf-8", 0); // Firefox/Thunderbird needs this - See STR#2930
        ...
        for(;;) {
            ...
            if(event.type == PropertyNotify) {
                ...
                do {
                    ...
                    memcpy(Data.data() + total, prop, num_bytes);
                    total += num_bytes;
                    if(prop)
                        XFree(prop);
                } while(bytes_after != 0);
    ...
    void Clipboard_Manager::handle_ClipboardType(const XEvent& e, const Atom types[], unsigned long count)
    {
        ...
        for(unsigned long i = 0; i < count; i++) {
            ...
            if(t == XaImageBmp || t == XaImagePNG) {
                ...
                return;
            } else if(t == Xatextplainutf || t == Xatextplainutf2 || t == Xatextplain || t == XaUtf8String ||
                      t == XaText || t == XaTextUriList || t == XaCompoundText) {
    ...
    void Clipboard_Manager::handle_SelectionRequest(const XEvent& ex)
    {
        ...
        if(ClipboardDataType_ == ClipboardDataType::TEXT) {
            if(e.target == TARGETS) {
                ...
                if(!Data.empty()) {
                    if(e.target == XaUtf8String || e.target == XA_STRING || e.target == XaCompoundText ||
                       e.target == XaText || e.target == Xatextplain || e.target == Xatextplainutf ||
                       e.target == Xatextplainutf2) {
                        if(e.target != XA_STRING)
                            e.target = XaUtf8String;
